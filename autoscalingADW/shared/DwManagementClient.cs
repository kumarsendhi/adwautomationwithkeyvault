...
namespace autoscalingADW.shared
{
    public class DwManagementClient
    {
        ...
        private string apiVersion = @"api-version=2017-03-01-preview";
        private string apiVersionScaling = @"api-version=2014-04-01-preview";
        ...
        public HttpResponseMessage ScaleWarehouse(string config, string location)
        {
            ...
            string scaleRestEndPoint = $"{restEndPointUrl}?{apiVersionScaling}";
        ...
        /// <summary>
        /// Gets User Activities in the DataWarehouse instance. 
        ...
        /// is no programmatic way to query DWH status as of now. 
        /// </summary>
        public HttpResponseMessage UserActivities()
        {
            string userActivityRestEndPoint = $"{restEndPointUrl}/dataWarehouseUserActivities/current?{apiVersion}";
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, userActivityRestEndPoint);
            request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", cloudCredentials.Token);
            HttpResponseMessage response = httpClient.SendAsync(request).Result;

            if (!response.IsSuccessStatusCode)
                throw new WebException($"Get User Activity operation failed with response from server {response.StatusCode}: {response.ReasonPhrase}");
